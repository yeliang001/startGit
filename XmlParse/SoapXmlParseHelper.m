...
+(NSString*)SoapMessageResultXml:(id)data ServiceMethodName:(NSString*)methodName{
    ...
    //NSString *searchStr=[NSString stringWithFormat:@"%@Response",methodName];
    //ax23:resultDetail
    NSString *searchStr = @"ns:return";
    if ([methodName rangeOfString:@"PushSurvey"].length>0) {
        searchStr = @"ax23:resultDetail";
    }
...
//    }
    for (GDataXMLNode * node in result) {
        MsgResult = [self getMsgResult:node searchStr:searchStr];
        if (![MsgResult isEqualToString:@""]) {
            break;
        }
    }
    [document release];
...
+(NSString *)getMsgResult:(GDataXMLNode *)node searchStr:(NSString *)searchStr{
    NSLog(@"-----%@-----",[node name]);
    if ([[node name] isEqualToString: searchStr]) {
            return [node stringValue];
    }else{
        if ([node childCount]>0) {
            for (GDataXMLNode *n in [node children]) {
                NSString * sT = [self getMsgResult:n searchStr:searchStr];
...
+(id)rootNodeToObject:(id)data objectName:(NSString*)className{
    id obj=[[NSClassFromString(className) new] autorelease];
    NSError *error=nil;
    GDataXMLDocument *document;
    if ([data isKindOfClass:[NSString class]]) {
        document=[[GDataXMLDocument alloc] initWithXMLString:data options:0 error:&error];
